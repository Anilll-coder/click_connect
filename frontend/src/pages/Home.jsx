...
export default function HomePage() {
  ...
  // -----------------------------
  // Load More (non-anonymous filtered)
  // -----------------------------
      ...
      <div className="text-center py-4">
        {posts.length > 0 && posts.length >= PAGE_LIMIT && (
          <button
            ...
            className="px-6 py-3 bg-gradient-to-r from-fuchsia-500 to-amber-400 text-white rounded-full shadow-md"
          >
            {loadingMore ? "Loading..." : "Load More Posts"}
          </button>
        )}
        {posts.length > 0 && posts.length === skip && !loadingMore && (
          <p className="text-gray-500 text-sm mt-3">You've reached the end of the feed!</p>
        )}
      </div>
    </div>
  );
